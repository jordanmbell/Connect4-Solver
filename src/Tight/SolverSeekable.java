...
public class SolverSeekable {

    public static void main (String[] args) {
        SolverSeekable s = new SolverSeekable(5, 5, 4);
    ...
    long[] logs = new long[7]; // setOffsets, rearrange, calculateLocation, isPrim, generateMoves, doMoves, Fileio
    /** Pieces stored in column major order, starting from bottom right*/
    public SolverSeekable(int w, int h, int wi) {
        width = w;
        height = h;
        win = wi;
    ...
    private Tuple<Primitive, Integer> solve(Piece[] position, int numPieces, Piece next, int m) {
        ...
        if (p.x != Primitive.NOT_PRIMITIVE) {
            try {
                ...
                channel.position(location);
                ByteBuffer buf = ByteBuffer.wrap(new byte[]{byteValue(p)});
                channel.write(buf);
        ...
        if (lossRemote != Integer.MAX_VALUE) {
            ...
            try {
                t = System.currentTimeMillis();
                channel.position(location);
                ByteBuffer buf = ByteBuffer.wrap(new byte[]{byteValue(temp)});
                channel.write(buf);
                logs[6] += System.currentTimeMillis() - t;
            } catch (Exception e) {
                throw new IllegalStateException("bad things happen");
